...
{question}<|eot_id|><|start_header_id|>assistant<|end_header_id|>""")
        ...
        def clean_response(text: str) -> str:
            stop_phrases = ["[공식 답변]", "[법적 근거]", "[기타 참고사항]", "[전문가의 추가 설명]", "[질문에 대한 추가 답변]", "[사용자에게 주는 추가 정보 및 조언]", "[전문가의 최종 진단 및 제안]", "[질문에 대한 완벽한 답변]", "사용자 질문:", "Human:", "답변:", "질문:", "끝."]
            pattern = re.compile(r'|'.join(map(re.escape, stop_phrases)), re.IGNORECASE)
            match = pattern.search(text)
            return text[:match.start()].strip() if match else text.strip()

        answer_chain = answer_prompt | self.llm | StrOutputParser() | RunnableLambda(clean_response)
...
async def ask_question(
    ...
    - 의존성 주입을 통해 RAGService를 받아옵니다.
    - 비동기로 동작하여 여러 요청을 효율적으로 처리합니다.
    """
